    ...
    "model_configs": [
      {
        ...
        "rate_limit_ids": ["rl-platform-rpm", "rl-platform-rpd"]
      },
      {
...
| `rate_limit_id` | No | Deprecated single reference to a legacy paired `governance.rate_limits` entry |
| `rate_limit_ids` | No | List of single-metric rate-limit IDs. Do not combine with `rate_limit_id`; an empty list clears model-owned links. |

For model limits, define one row per metric/window in `governance.rate_limits`. Set `metric` to `requests` or `tokens`; the row must contain only the corresponding `request_*` or `token_*` fields. This supports combinations such as `15 RPM + 1,500 RPD + TPM + TPD`. Existing legacy `rate_limit_id` rows remain supported.
