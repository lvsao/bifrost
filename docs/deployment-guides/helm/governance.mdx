...
| `rate_limit_id` | — | Deprecated single reference to a legacy paired `governance.rate_limits` entry |
| `rate_limit_ids` | — | List of single-metric rate-limit IDs; do not combine with `rate_limit_id` |

Model limits can attach multiple single-metric rate-limit rows, each with a different window. Use `metric: requests` or `metric: tokens` in the referenced `governance.rate_limits` rows to express RPM/RPD/TPM/TPD combinations.
